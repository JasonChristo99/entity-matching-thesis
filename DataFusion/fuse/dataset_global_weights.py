...
class DatasetGlobalWeights:
    ...
    def __init__(self, fuse_env, name, entity_attributes,
        ...
        self.src_weights = {k: 1.0 for k in self.sources}
        self.src_acc = {k: 1.0 for k in self.sources}
    ...
    def train_matchers(self, matched_facts=None):
        ...
        for attr in self.entity_attribute_matchers:
            premached_facts = None if matched_facts is None else matched_facts.get(attr, None)
            self.entity_attribute_matchers[attr].train(premached_facts, 200)

    def match_observations(self):
        """
        Iterate over collections and retrieve clusters of matching osbserved facts.
        :return: None
        """
    ...
    def find_true_clusters(self):
        ...
        fuser = TruthDiscoveryGlobalWeights(self.env, votes, self.src_weights)
    ...
    def find_true_facts(self, no_weights=False):
        ...
        for cluster in self.true_clusters:
            cfact = CanonicalFact(self.env, cluster, self.src_acc)
            self.canonical_facts.add(cfact)
        # Call fusion with true clusters and src accuracy
        fuser = FuseObservationsGlobalWeights(self.env, self.canonical_facts, self.src_acc, no_weights)
        fuser.run()
    ...
    def get_src_frame(self):
        ...
        """
        return pd.DataFrame(self.src_weights.items(), columns=['source', 'weight'])
